...
///
/// Local DSP only — no models, no downloads, no network (PROJECT_MASTER 3.7,
/// ADR 0004). Implementations run on a low-priority background thread and must
/// remain interruptible: analysing a five-thousand-track library must never make
/// playback stutter or the machine feel busy (PROJECT_MASTER 2.11).
