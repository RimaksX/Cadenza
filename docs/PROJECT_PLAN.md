...
| M15 | Финальный дизайн | done |
| M16 | Packaging и стабилизация | not started |

M16 несёт четыре вещи, отложенные раньше и записанные, чтобы не потеряться:
`deny.toml` и проверка зависимостей в CI (отложены с M0, когда зависимостей ещё
не было, — теперь их сотни); джоба CI, собирающая на объявленной минимальной
версии 1.92, потому что `stable` её не проверяет; `strip` в профиле релиза,
иначе установщик потащит отладочные символы; и решение про поле `repository` в
`Cargo.toml`, которое сейчас указывает на несуществующий адрес.
...
Задачи мастера: analysis jobs, BPM, key, energy, spectral features, cache, low
priority worker. DoD: анализ не блокирует UI, фичи сохраняются, повторный анализ
не происходит.

Что построено:

- **`infra/analysis`** — окно из середины файла (`decode`), одно преобразование
  на всех (`dsp`), и по нему: громкость и динамика (`energy`), яркость
  (`spectral`), темп (`bpm`), тональность (`key`); `danceability` и `valence`
  выводятся из уже измеренного. `feature_extractor` собирает строку, `worker`
  задаёт темп работы.
- **`core`** — `TrackFeaturesRepositoryPort`, `analysis_policy` (доля машины,
  размер партии, приоритет), `AnalysisService`: поставить в очередь, выполнить
  один шаг, отчитаться о прогрессе. Ничего не блокирует и не спит.
- **`infra/db`** — `analysis_repo` и `features_repo`. Миграции не понадобились:
  обе таблицы существуют с 3-й и 11-й (`MASTER_ISSUES` 47).
- **`app`** — воркер живёт столько же, сколько окно, и `cadenza analyze [n]` для
  проверки руками.

Алгоритмы: спектральный поток → автокорреляция для темпа, хрома →
Крумхансль-Шмуклер для тональности, RMS и перцентили для громкости и динамики.
Ни одной модели, ничего не скачивается — 12.1 соблюдён архитектурно, а не
обещанием.

Измерено на реальных файлах: 151 мс на трек, то есть 13 минут одного ядра на
пять тысяч треков. Слушать можно сразу; анализ идёт позади и держит свою долю
машины сам.

## M12 — Smart shuffle

Задачи мастера: transition score, cooldowns, no repeat pool, fallback. DoD:
shuffle умнее random, нет частых повторов.

Оценка перехода (9.3) была написана ещё в M1 — M12 добавляет сам выбор (9.4):

- `shuffle_policy::choose_next` — кандидаты, вето по правилам 9.2, оценка
  каждого против того, что играет, лучшая горстка (15) и взвешенный жребий
  среди них.
- `QueueService::shuffled_successor` — круг (то, что ещё не звучало), сброс
  круга при repeat all, чтение фич один раз на смену трека.
- `playback_policy::next_in_library` стал только про порядок.

Fallback честный и не отдельный: трек без анализа получает нейтральную оценку и
свою очередь, а библиотека без анализа вообще перемешивается ровно как в M7.

## M13 — Smart Radio

Задачи мастера: mood presets, radio session, candidate generation, ranking,
infinite queue, feedback. DoD: можно выбрать mood, радио генерирует поток, скипы
влияют на подбор.

- **Настроения** — восемь встроенных с полосами (миграция 17), свои сохраняются
  и удаляются, встроенные не переписываются. Полосы мягкие: жёсткая отсечка в
  маленькой библиотеке — это настроение, которое ничего не выдаёт.
- **Ранжирование** — `radio_policy::rank`, шесть слагаемых 10.4. Штраф за
  однообразие вычитается, шум — амплитуда (`MASTER_ISSUES` 49).
- **Сессия** — `RadioService`: партия 8–15, по одному выбору за раз, у каждого
  записана причина; очередь пополняет полосу при остатке в 3 и держит ручную
  очередь впереди.
- **Обратная связь** — миграция 18, вердикт лежит на конкретном предложении и
  суммируется по сессиям. Скип отправляется, когда «дальше» нажали руками.
- **Экран** — восемь карточек с тем, что настроение просит, «более такого» и
  «менее». Кнопки остановки нет: станция кончается, когда играет что-то другое
  (`MASTER_ISSUES` 50).

Свежесть на момент M13 считалась по собственным выборам радио: истории
прослушиваний никто не писал. После M14 источник расширен — берётся более поздний
из двух ответов, «когда предлагали» и «когда слушали», — а формула осталась той
же (`MASTER_ISSUES` 49, 61).

## M14 — Аналитика и retention

Задачи мастера: play events, aggregates, retention 30 days, purge job, analytics
view. DoD: статистика собирается, история очищается, история отключается
полностью.

- **Запись** — `PlaybackService` открывает прослушивание, когда трек загружен, и
  закрывает, когда его сменил следующий; позиция снимается до `engine.load()`,
  иначе она уже нулевая. `history_policy` решает, чем это было: больше половины
  — прослушано, меньше десяти секунд — скип, между ними — ни то ни другое.
- **Выключатель настоящий** — история пишется, только если профиль это разрешил,
  а по умолчанию он не разрешает (1.4). Выключение стирает написанное, а не
  перестаёт его показывать.
- **Retention** — тридцать дней держатся удалением, а не условием в запросе: при
  запуске приложение чистит просроченное у каждого профиля, а не только у
  активного. То, что лишь скрыто, — всё ещё есть.
- **Счёт** — `StatsRepositoryPort`: одна строка сводки и десять самых
  слушаемых, обе считаются запросом по `play_events`. Таблицы `daily_*` из 7.4
  созданы миграцией 7 и не пишутся никем (`MASTER_ISSUES` 54).
- **Экран** — «Listening»: четыре числа, десять строк рейтинга и два разных
  пустых состояния — «история выключена» и «за месяц ничего не сыграно» — потому
  что это разные вещи и лечатся они по-разному.

## После M14 — три требования, у которых не было милстоуна

Найдены проверкой кода против раздела 2, а не против списков задач
(`MASTER_ISSUES` 51). Владелец решил закрыть их до M15 и M16.

- **Переключение слушателя.** `ProfileService` умел это с M3, окно — нет.
  Добавлены раздел «WHO IS LISTENING» в настройках, `QueueService::reload` и
  `Controller::switch_profile`: остановить, переключить, перечитать очередь
  нового профиля, закончить станцию, обновить все экраны. Очередь при этом
  теряет продолжение библиотеки — оно вычисляется, а не хранится.
- **Правка тегов трека.** `LibraryService::edit_track` пишет локальное
  переопределение в `profile_tracks` (2.1), а не в файл: исполнитель и альбом
  ищутся по имени, пустое значение означает отсутствие, пустое название
  отвергается. В окне — `EditTrackDialog` из меню строки.
- **Экран решений.** Дубликат находился с M4, записывался и никогда не
  показывался. `LibraryService::review_cards` отдаёт, где файл, почему он ждёт и
  с чем столкнулся; экран предлагает три из четырёх вариантов 2.1, а четвёртый —
  правка тегов — это тот же редактор, потому что править нечего, пока файла нет
  в библиотеке. Пункт в боковой панели появляется, только пока что-то ждёт.

## После M14 — библиотека, которая следит за собой

Проверка того, что действительно работает, а не выглядит работающим
(`MASTER_ISSUES` 55), нашла невыполненное требование: «Автоматическое
отслеживание изменений файловой системы» держалось только командой
`cadenza watch`, то есть пока рядом с окном открыт терминал. Наблюдатель был
построен и покрыт тестами ещё в M4 и никогда не подключался к окну.

Подключён вместе со второй половиной той же истории — шиной событий, у которой
было шестнадцать мест публикации и ни одного подписчика (`MASTER_ISSUES` 56).

- `LibraryPorts` получил необязательный `FileWatcherPort`; наблюдателя создаёт
  только `Command::Ui`. Папка, добавленная в библиотеку, ставится под наблюдение,
  удалённая — снимается, а те, что были раньше, берутся при старте окна.
- Окно узнаёт о переменах флагом, а не событием: подписчик работает в потоке
  наблюдателя, а Slint можно трогать только из цикла событий. Тик в 250 мс,
  который и так читает транспорт, читает и флаг. Заодно бесплатно склеиваются
  всплески: скопированный альбом публикует по событию на файл, и пятьсот флагов —
  это одно обновление.
- Обновляются три вещи из девяти: библиотека, очередь и решения. Эквалайзер, тему
  и месяц прослушиваний файл на диске изменить не может.

## После M14 — окно принимает файлы, и три гарнитуры сменились

- **Перетаскивание из проводника.** Slint такие события не доставляет, хотя winit
  их сообщает; добрались через опубликованный шов
  `with_custom_application_handler` — без Win32 и без `unsafe`
  (`MASTER_ISSUES` 57). Пока файл над окном, всё за ним гаснет и надпись называет
  правила: папка входит в библиотеку и встаёт под наблюдение, файл берётся там,
  где лежит. Импорт идёт своим потоком, ответ возвращается строкой в плеере.
  Перетаскивание *внутри* окна отложено: в Slint 1.13 оно экспериментальное
  (`MASTER_ISSUES` 58).
- **Гарнитуры.** Instrument Serif не имел кириллицы вовсе — русское название
  проваливалось в системную подстановку. Теперь Cormorant Garamond и Manrope,
  JetBrains Mono остался. Все три переименованы в «Cadenza Serif/Sans/Mono»,
  потому что системная копия шрифта с тем же именем побеждает встроенную
  (`MASTER_ISSUES` 59).

## Из M16, раньше срока — лог

Задача «logs» перенесена вперёд, потому что появилась причина: за окном теперь
работают три пути — анализатор, наблюдатель за файлами и импорт с перетаскивания,
— и каждый из них молча продолжает работу вместо того, чтобы прерывать
слушателя. `LogPort` живёт на `AppContext`, `FileLog` пишет в
`%LOCALAPPDATA%/Cadenza/logs/app.log` и перекатывает его на мегабайте, храня одно
предыдущее поколение. Ни уровней-переключателей, ни фильтров, ни зависимости
(`MASTER_ISSUES` 63).

## После M15 — как музыка попадает внутрь

Разбор по просьбе владельца: не залатать три жалобы, а продумать все сценарии
(`MASTER_ISSUES` 68, 69).

- **Сканирование при запуске** — потоком, не задерживая окно. Наблюдатель знает
  только то, что случилось при нём; музыка, положенная при закрытом плеере, до
  этого ждала ручного нажатия.
- **SCAN NOW отчитывается**: добавлено, перечитано, уже было, пропало, ждёт
  решения. Прежде отчёт выбрасывался, и «не нашёл» выглядело как «не сработал».
- **TAKEN OUT** в настройках — что этот слушатель убрал, с возвратом на строке.
  Убранное было решением без отмены и без следа: файл на диске, в наблюдаемой
  папке, а библиотека его не берёт.
- **SYNC на папке** — привести библиотеку в соответствие с папкой: вернуть
  убранное, убрать то, чего на диске нет. Запись в каталоге и история остаются.
- **Строка о треках извне** — брошенный на окно файл берётся по месту, и никакая
  папка за него не отвечает.

И дефект, который всё это вскрыло: быстрый путь сканирования не писал состояние
файла, а `refresh_missing` — проход, написанный ровно для этого, — не вызывался
ниоткуда с M4. Файл, вернувшийся на место, оставался «missing» навсегда.

## M15 — что от него осталось

Входной документ не придёт: дизайн сложился по ходу работы и владелец его
утвердил (`MASTER_ISSUES` 53). Написан он теперь как
[docs/design/DESIGN_PRINCIPLES.md](design/DESIGN_PRINCIPLES.md) — описание того,
что уже на экране; нормативны по-прежнему токены.

Пять задач из семи сделаны давно: токены, библиотека компонентов, EqStar, стиль
визуализатора, тёмная и светлая темы. Не сделаны две, и обе — требования 2.10
сами по себе:

- **масштабирование интерфейса** — четыре ступени в настройках, у профиля,
  применяются сразу: масштаб живёт в длинах, а не в рендерере (`MASTER_ISSUES`
  64, 65, 66);
- **drag-and-drop треков** — трек тащится в очередь, цель подсвечивается. Ради
  этого Slint поднят до 1.17, где `DragArea`/`DropArea` стабильны
  (`MASTER_ISSUES` 58, 67).

## Перед M16 — подготовка к публикации

Три решения, принятые владельцем перед тем, как заливать репозиторий заново
(`MASTER_ISSUES` 75).

- **Лицензия — `GPL-3.0-only`.** Slint даётся под версией 3 и никакой другой,
  так что «or later» обещало условия, под которые часть работы перейти не может.
  Текст `LICENSE` тот же, обязательства те же; изменилось только объявление —
  в `Cargo.toml`, в README и в ADR 0002.
- **Консоли нет.** Бинарник собран под подсистему windows: у плеера, открытого
  двойным щелчком, должно быть окно и ничего больше. Проверено по заголовку PE,
  а не на глаз: subsystem 2, не 3. Всё, что раньше печаталось, идёт в лог; два
  отказа, при которых приложения не существует, — в диалог через `rfd`.
- **Командной строки нет.** `crates/app/src/cli.rs` с самого начала был
  строительными лесами «до появления UI» и пережил M6 на девять милстоунов.
  Каждый его глагол умеет интерфейс, кроме удаления профиля, которого нет ни в
  одном требовании. 1546 строк `crates/app` стали 300; тестов 496 вместо 505 —
  ушли девять тестов разбора аргументов.

Поле `repository` вернулось: теперь есть куда указывать. У всех крейтов стоит
`publish = false`.

Осталось для M16: `deny.toml` и проверка зависимостей в CI, иконка приложения,
WiX manifest, install/uninstall, performance pass, финальные тесты.

## После M15 — трек по ссылке

Владелец попросил приносить трек по вставленной ссылке. Правило 12.1 это
запрещало — и оказалось, что запрет был не тем, что имелось в виду: замысел был
*локальный* плеер (треки лежат у тебя, ничто не зависит от сервиса), а строку
«не добавляй интернет» скорее всего внесла нейросеть при проверке плана.
Раздел 1.4 мастера переписан на «Локальность и приватность», подробности —
`MASTER_ISSUES` 76.

- `core`: порт `FetchPort`, политика `link_policy` (то, что вводят, становится
  аргументом чужой программы — значит проверяем только то, что оно не может
  стать чем-то другим), `LibraryService::fetch_from_link` с тремя исходами:
  положено, нужна папка, нужны программы.
- `infra`: `ExternalFetcher` — запускает `yt-dlp` и `ffmpeg`, установленные
  пользователем. Ни одна из них не поставляется с Cadenza. Скачивание идёт во
  временную папку, за которой никто не следит, и в библиотеку переезжает только
  готовый mp3.
- `ui`: строка с полем и кнопкой GET в шапке библиотеки, проценты в самой
  кнопке, и `MAKE A FOLDER` там, где класть некуда.

Cadenza по-прежнему не открывает соединений сама и не содержит сетевого клиента.


## M16 — что уже сделано

- **Иконка.** Мастер-файл в `resources/branding/cadenza.svg`, семь размеров
  в `resources/branding/cadenza.ico`, каждый отрисован в своём размере, а не
  уменьшен из одного большого. Вкомпилирована в exe через `crates/app/build.rs`
  — иначе Windows рисует пустой лист. Проверено по самому бинарнику.
  Известное ограничение: в 16 пикселей знак не читается (`MASTER_ISSUES` 81).
- **Установщик.** `packaging/windows/` — манифест WiX, две картинки в палитре
  и гарнитурах проекта, лицензия в RTF, инструкция сборки. Ставится для одного
  пользователя без прав администратора, при удалении не трогает библиотеку.
- **`deny.toml` и джоба CI** — список лицензий снят с дерева, а не придуман.

Осталось: собрать MSI и проверить установку и удаление на чистой машине,
performance pass, финальные тесты. И подпись кода — без неё SmartScreen
встречает установщик синим окном, и это влияет на доверие сильнее оформления.
